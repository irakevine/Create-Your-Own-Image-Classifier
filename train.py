...
def parse_args():
    ...
    parser.add_argument(
        "--hidden_units", action="store", dest="hidden_units", type=int, default=512
    )
...
def train_model(
    epochs, trainloader, validateloader, model, device, criterion, optimizer
):
    steps = 0
    running_loss = 0
    print_every = 5
    ...
    print(
        " Model Trained in: {:.0f}m {:.0f}s".format(total_time // 60, total_time % 60)
    )
...
def main():
    ...
    train_model(
        args.epochs, dataloaders[0], dataloaders[1], model, device, criterion, optimizer
    )
